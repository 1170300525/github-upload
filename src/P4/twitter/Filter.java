...
public class Filter {
    ...
    public static List<Tweet> containing(List<Tweet> tweets, List<String> words) {
    	...
    	int size = tweets.size();
    	for (int i = 0; i < size; i++) {
    		String[] str = tweets.get(i).getText().split(" ");
    		for (String ptr : str) {
    			for (String s : words) {
    				if (ptr.toUpperCase().equalsIgnoreCase(s.toUpperCase()))
    					contain.add(tweets.get(i));
    			}
    		}
    	}
		return contain;
